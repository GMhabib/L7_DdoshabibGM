    ...
    async function main() {
        ...
        for (let i = 0; i < threads; i++) {
            ...
            worker.on('message', (results) => {
                ...
                let condition = "GOOD";
                if (successRate < 80 || avgLat > 1000) condition = "WARNING";
                if (successRate < 50 || avgLat > 3000) condition = "BAD";

                let methodLog = Object.keys(methodStats)
                    .filter(k => (methodStats[k].ok + methodStats[k].err) > 0)
                    .map(k => `${k}(OK:${methodStats[k].ok} ERR:${methodStats[k].err})`)
                    .join(' | ');

                process.stdout.write(`\r[${condition}] Total: ${successfulRequests} | Latency: ${avgLat}ms | ${methodLog} `);
            });
        }
